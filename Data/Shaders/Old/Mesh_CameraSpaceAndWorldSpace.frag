...
void ComputeLightType(LeLight light, inout vec3 fragToLight, inout float attenuation)
{
	if (light.position.w > 0.0)	// Punctual light
	{
		...
		{
			vec3 position = params.useCameraSpace ? (ubo.view * light.position).xyz : light.position.xyz;
			fragToLight = position - inPos;
			...
			coneDirection = normalize(params.useCameraSpace ? (ubo.view * vec4(coneDirection, 0.0)).xyz : coneDirection);
			...
			if (light.useSoftEdge)
			{
				...
				float epsilon = max(cos(radians(light.innerAngle)) - cos(radians(light.outerAngle)), 0.001);
				attenuation *= clamp((theta - cos(radians(light.outerAngle))) / epsilon, 0.0, 1.0); // use mix() instead?
			}
			else
			{
	...
	else // Directionnal Light
	{
		// Invert cause direction is a direction pointing from the light source
		vec3 direction = RotateVectorByVector(vec3(0.0, 0.0, 1.0), light.rotation.xyz);
		fragToLight = params.useCameraSpace ? (ubo.view * vec4(direction, 0.0)).xyz : direction;
...
vec3 Refract(vec3 normal, vec3 V)
{
...
void main() 
{
	...
	vec3 V = normalize(inEyePos - inPos);
	...
	if(o.metallic > 0.f)
	{
		finalColor += Refract(o.normal, V) * o.metallic;
	}
